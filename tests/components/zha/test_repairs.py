...
def make_firmware_probe_result(firmware_type: FirmwareType) -> FirmwareInfo:
    """Create a firmware probe result."""
    return FirmwareInfo(
        device="/dev/ttyUSB0",
...
async def test_multipan_firmware_repair(
    ...
    with (
        patch(
            "homeassistant.components.zha.repairs.wrong_silabs_firmware.probe_silabs_firmware",
            return_value=make_firmware_probe_result(FirmwareType.MULTIPROTOCOL),
            autospec=True,
        ),
        patch(
    ...
    assert issue.translation_placeholders["firmware_type"] == "MULTIPROTOCOL"
...
async def test_multipan_firmware_no_repair_on_probe_failure(
    ...
    with (
        patch(
            "homeassistant.components.zha.repairs.wrong_silabs_firmware.probe_silabs_firmware",
            side_effect=FirmwareProbingFailed(),
            autospec=True,
        ),
        patch(
            "homeassistant.components.zha.Gateway.async_initialize",
...
async def test_multipan_firmware_retry_on_probe_ezsp(
    ...
    with (
        patch(
            "homeassistant.components.zha.repairs.wrong_silabs_firmware.probe_silabs_firmware",
            return_value=make_firmware_probe_result(FirmwareType.ZIGBEE),
            autospec=True,
        ),
        patch(
            "homeassistant.components.zha.Gateway.async_initialize",
