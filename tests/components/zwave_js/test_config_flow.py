...
def mock_scan_serial_ports_fixture(serial_port: USBDevice) -> Generator[MagicMock]:
    ...
    with patch(
        "homeassistant.components.zwave_js.config_flow.usb.scan_serial_ports"
    ) as mock_scan:
        ...
        mock_scan.return_value = [serial_port, another_port]
        yield mock_scan
...
async def test_get_usb_ports_single_valid_port() -> None:
    ...
    with patch(
        ...
        assert list(result.values()) == [
            "Device A - /dev/ttyUSB0, s/n: n/a - 10C4:EA60",
        ]
